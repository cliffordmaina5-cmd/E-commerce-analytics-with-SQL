...
SELECT COUNT(DISTINCT item_key) AS unique_products_sold
FROM fact_table;
...
FROM fact_table f
INNER JOIN store_dim s ON f.store_key = s.store_key
GROUP BY s.district
ORDER BY total_revenue DESC;
...
INNER JOIN store_dim s ON f.store_key = s.store_key
INNER JOIN item_dim i ON f.item_key = i.item_key
INNER JOIN top_district_per_year tdy ON tdy.year = t.year AND tdy.district = s.district
ORDER BY t.year;
...
FROM fact_table f
INNER JOIN trans_dim tr ON f.payment_key = tr.payment_key
